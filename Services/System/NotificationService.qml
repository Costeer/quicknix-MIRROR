...
Singleton {
  ...
  property string historyFile: Quickshell.env("QUICKNIX_NOTIF_HISTORY_FILE") || (Settings.cacheDir + "notifications.json")

  // State
  property real lastSeenTs: 0
  // Volatile property that doesn't persist to settings (similar to quicknixPerformanceMode)
  ...
  function handleNotification(notification) {
    ...
    if (root.doNotDisturb || PowerProfileService.quicknixPerformanceMode)
      return;
