...
# Push translations to QuickNix Translate API
...
# Configuration
API_URL="${TRANSLATION_API_URL:-https://i18n.quicknix.dev}"
PROJECT_SLUG="${TRANSLATION_PROJECT:-quicknix-shell}"

# Check for secret
if [ -z "$QUICKNIX_SHELL_TRANSLATION_PUSH_SECRET" ]; then
    echo "Error: QUICKNIX_SHELL_TRANSLATION_PUSH_SECRET environment variable is required"
    exit 1
fi
...
RESPONSE=$(echo "$COMBINED_JSON" | curl -s -w "\n%{http_code}" -X POST \
    "$PUSH_URL" \
    -H "Authorization: Bearer $QUICKNIX_SHELL_TRANSLATION_PUSH_SECRET" \
