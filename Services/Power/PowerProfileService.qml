...
Singleton {
  ...
  property bool quicknixPerformanceMode: false
  ...
  // QuickNix Performance Mode
  // - Turning shadow off
  // - Turning animation off
  // - Do Not Disturb
  function toggleQuickNixPerformance() {
    quicknixPerformanceMode = !quicknixPerformanceMode;
  }

  function setQuickNixPerformance(value) {
    quicknixPerformanceMode = value;
  }

  onQuicknixPerformanceModeChanged: {
    if (quicknixPerformanceMode) {
      ToastService.showNotice(I18n.tr("toast.quicknix-performance.label"), I18n.tr("toast.quicknix-performance.enabled"), "rocket");
    } else {
      ToastService.showNotice(I18n.tr("toast.quicknix-performance.label"), I18n.tr("toast.quicknix-performance.disabled"), "rocket-off");
    }
  }
}
