...
SmartPanel {
  ...
  panelContent: Item {
    ...
    Keys.onPressed: event => {
                      if (event.key === Qt.Key_P) {
                        if (powerProfileAvailable) {
                          var nextIdx = (profileIndex + 1) % powerProfiles.length;
                          setProfileByIndex(nextIdx);
                          profileKeyHint.active = true;
                        }
                        event.accepted = true;
                        return;
                      }
                      if (event.key === Qt.Key_T) {
                        if (showQuickNixPerformance) {
                          PowerProfileService.quicknixPerformanceMode = !PowerProfileService.quicknixPerformanceMode;
                          perfKeyHint.active = true;
                        }
                        event.accepted = true;
                        return;
                      }
    ...
    readonly property bool showQuickNixPerformance: panelID ? panelID.showQuickNixPerformance : resolveWidgetSetting("showQuickNixPerformance", false)
    ...
    ColumnLayout {
      ...
      NBox {
        ...
        visible: BatteryService.laptopBatteries.length > 0
        ...
        ColumnLayout {
          ...
          // Other devices (Bluetooth) section - disabled (BluetoothService removed)
          Repeater {
            model: []
            delegate: ColumnLayout {
              Layout.fillWidth: true
      ...
      NBox {
        ...
        visible: showPowerProfiles || showQuickNixPerformance
        ...
        ColumnLayout {
          ...
          ColumnLayout {
            ...
            NValueSlider {
              ...
              onSliderPressedChanged: (pressed, v) => {
                                  if (!pressed) {
                                    setProfileByIndex(v);
                                  }
                                }
          ...
          NDivider {
            Layout.fillWidth: true
            visible: showPowerProfiles && PowerProfileService.available && showQuickNixPerformance
          }

          RowLayout {
            Layout.fillWidth: true
            spacing: Style.marginS
            visible: showQuickNixPerformance

            NText {
              text: I18n.tr("toast.quicknix-performance.label")
            ...
            NIcon {
              icon: PowerProfileService.quicknixPerformanceMode ? "rocket" : "rocket-off"
              pointSize: Style.fontSizeL
              color: PowerProfileService.quicknixPerformanceMode ? Color.mPrimary : Color.mOnSurfaceVariant
            }

            NToggle {
              checked: PowerProfileService.quicknixPerformanceMode
              onToggled: checked => PowerProfileService.quicknixPerformanceMode = checked
            }
