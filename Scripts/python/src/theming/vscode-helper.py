#!/usr/bin/env python3
# Finds all installed QuickNix theme extensions for VSCode/VSCodium.

import sys
from pathlib import Path


def find_all_quicknix_themes(extensions_dir: Path, prefix: str) -> list[str]:
    ...
    return [str(d / "themes" / "QuickNixTheme-color-theme.json") for d in candidates]


if __name__ == "__main__":
    # Resolve ~ in the provided extensions directory path
    extensions_dir = Path(sys.argv[1]).expanduser()
    prefix = sys.argv[2] if len(sys.argv) > 2 else "quicknix.quicknixtheme-"

    # Print the resolved paths to stdout for the QML Process to capture
    results = find_all_quicknix_themes(extensions_dir, prefix)
    if results:
        for path in results:
            print(path)
    else:
