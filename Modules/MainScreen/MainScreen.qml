...
PanelWindow {
  ...
  WlrLayershell.namespace: "quicknix-background-" + (screen?.name || "unknown")
  ...
  Instantiator {
    model: Settings.data.general.keybinds.keyOpenAudio || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("audioPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenBrightness || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("brightnessPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenBattery || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("batteryPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenNotifications || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("notificationHistoryPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenClock || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("clockPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenTray || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("trayDrawerPanel")
    }
  }

  Instantiator {
    model: Settings.data.general.keybinds.keyOpenWifi || []
    Shortcut {
      sequence: modelData
      enabled: !PanelService.isKeybindRecording
      onActivated: root.togglePanel("networkPanel")
    }
  }
}
