...
SmartPanel {
  ...
  panelContent: Item {
    ...
    function adjustFirstSlider(delta) {
      if (globalBrightnessSlider.enabled && panelContent.globalBrightnessCapableMonitors > 1 && panelContent.resolveWidgetSetting("applyToAllMonitors", false)) {
        var newVal = Math.max(0, Math.min(1, panelContent.globalBrightness + delta));
        panelContent.globalBrightness = newVal;
        panelContent.applyGlobalBrightness(newVal);
      } else {
        var monitors = getControllableMonitors();
        if (monitors.length > 0) {
          var m = monitors[0];
          var newVal = Math.max(0, Math.min(1, (isNaN(m.brightness) ? 0 : m.brightness) + delta));
            ...
            ColumnLayout {
              ...
              RowLayout {
                ...
                NValueSlider {
                  ...
                  onSliderPressedChanged: (pressed, value) => {
