...
def ensure_gtk_css_import(gtk_css: Path, colors_file: Path, label: str) -> bool:
    ...
    if gtk_css.exists() or gtk_css.is_symlink():
        ...
        if "quicknix.css" in content and "@import" in content:
            return True
        ...
        print(f"Appended {label} quicknix.css import to gtk.css")
    else:
        gtk_css.write_text(GTK_IMPORT + "\n")
        print(f"Created {label} gtk.css with quicknix.css import")
    return True


async def apply_gtk3_colors(config_dir: Path):
    gtk3_dir = config_dir / "gtk-3.0"
    colors_file = gtk3_dir / "quicknix.css"
...
async def apply_gtk4_colors(config_dir: Path):
    ...
    colors_file = gtk4_dir / "quicknix.css"
...
async def main():
    ...
    if all(results):
        ...
        # gtk.css / quicknix.css setup failed.
        await sync_system_appearance(mode, update_gtk_theme=False)
        sys.exit(1)
