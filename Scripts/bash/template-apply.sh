...
kitty)
    ...
    if [ -w "$KITTY_CONF" ]; then
        kitty +kitten themes --reload-in=all quicknix
    else
...
ghostty)
    ...
    for CONFIG_FILE in "${CONFIG_FILES[@]}"; do
        if [ -f "$CONFIG_FILE" ]; then
            FOUND_CONFIG=true
            # Check if theme is already set to quicknix (flexible spacing)
            if grep -qE "^theme\s*=\s*quicknix$" "$CONFIG_FILE"; then
                : # Already correct
            elif grep -qE "^theme\s*=" "$CONFIG_FILE"; then
                # Replace existing theme line in-place
                sed -i -E 's/^theme\s*=.*/theme = quicknix/' "$CONFIG_FILE"
            else
                # Add the new theme line to the end of the file
                echo "theme = quicknix" >>"$CONFIG_FILE"
            fi
        fi
    done
...
foot)
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Create the config file with the quicknix theme
        cat >"$CONFIG_FILE" <<'EOF'
[main]
include=~/.config/foot/themes/quicknix
EOF
    else
        # Check if theme is already set to quicknix
        if ! grep -q "include.*quicknix" "$CONFIG_FILE"; then
            ...
            if grep -q '^\[main\]' "$CONFIG_FILE"; then
                ...
                sed -i '/^\[main\]/a include=~/.config/foot/themes/quicknix' "$CONFIG_FILE"
            else
...
alacritty)
    ...
    NEW_THEME_PATH='~/.config/alacritty/themes/quicknix.toml'

    # Check if the config file exists, create it if it doesn't.
    if [ ! -f "$CONFIG_FILE" ]; then
        # Create the config directory if it doesn't exist
        mkdir -p "$(dirname "$CONFIG_FILE")"
        # Create the config file with the quicknix theme import
        cat >"$CONFIG_FILE" <<'EOF'
[general]
import = [
    "~/.config/alacritty/themes/quicknix.toml"
]
EOF
    else
        # Check if quicknix theme is already imported (any path variant)
        if grep -q 'quicknix\.toml' "$CONFIG_FILE"; then
            # Update old relative path to new absolute path if needed
            if grep -q '"themes/quicknix.toml"' "$CONFIG_FILE"; then
                sed -i 's|"themes/quicknix.toml"|"'"$NEW_THEME_PATH"'"|g' "$CONFIG_FILE"
            fi
            # Already has quicknix import with correct path, nothing to do
        else
            # No quicknix import found, add it
...
wezterm)
    ...
    WEZTERM_SCHEME_LINE='config.color_scheme = "QuickNix"'

    # Check if the config file exists.
    if [ -f "$CONFIG_FILE" ]; then

        # Check if theme is already set to QuickNix (matches 'QuickNix' or "QuickNix")
        if ! grep -q "^\s*config\.color_scheme\s*=\s*['\"]QuickNix['\"]\s*" "$CONFIG_FILE"; then
            # Not set to QuickNix. Check if *any* color_scheme line exists.
            if grep -q '^\s*config\.color_scheme\s*=' "$CONFIG_FILE"; then
                # It exists, so we replace it with our desired line.
                sed -i "s|^\(\s*config\.color_scheme\s*=\s*\).*$|\1\"QuickNix\"|" "$CONFIG_FILE"
            else
...
fuzzel)
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Create the config file with the quicknix theme
        cat >"$CONFIG_FILE" <<'EOF'
include=~/.config/fuzzel/themes/quicknix
EOF
    else
        # Check if theme is already set to quicknix
        if grep -q "^include=~/.config/fuzzel/themes/quicknix$" "$CONFIG_FILE"; then
            : # Already correct
        elif grep -q "^include=.*themes" "$CONFIG_FILE"; then
            # Replace existing theme include line in-place
            sed -i 's|^include=.*themes.*|include=~/.config/fuzzel/themes/quicknix|' "$CONFIG_FILE"
        else
            # Add the new theme include line
            echo "include=~/.config/fuzzel/themes/quicknix" >>"$CONFIG_FILE"
        fi
    fi
    ;;
...
cava)
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        if grep -q '^\[color\]' "$CONFIG_FILE"; then
            # Check if theme is already set to quicknix under [color] (flexible spacing)
            if sed -n '/^\[color\]/,/^\[/p' "$CONFIG_FILE" | grep -qE '^theme\s*=\s*"quicknix"'; then
                : # Already correct
            elif sed -n '/^\[color\]/,/^\[/p' "$CONFIG_FILE" | grep -qE '^theme\s*='; then
                # Replace existing theme line under [color]
                sed -i -E '/^\[color\]/,/^\[/{s/^theme\s*=.*/theme = "quicknix"/}' "$CONFIG_FILE"
                THEME_MODIFIED=true
            else
                # Add theme line after [color]
                sed -i '/^\[color\]/a theme = "quicknix"' "$CONFIG_FILE"
                THEME_MODIFIED=true
            fi
        else
...
EOF
        ...
        if grep -q '^\[flavor\]' "$CONFIG_FILE"; then
            ...
            if sed -n '/^\[flavor\]/,/^\[/p' "$CONFIG_FILE" | grep -q '^dark\s*='; then
                sed -i '/^\[flavor\]/,/^\[/{s/^dark\s*=.*/dark  = "quicknix"/}' "$CONFIG_FILE"
            else
                sed -i '/^\[flavor\]/a dark  = "quicknix"' "$CONFIG_FILE"
            fi
            if sed -n '/^\[flavor\]/,/^\[/p' "$CONFIG_FILE" | grep -q '^light\s*='; then
                sed -i '/^\[flavor\]/,/^\[/{s/^light\s*=.*/light = "quicknix"/}' "$CONFIG_FILE"
            else
                sed -i '/^\[flavor\]/,/^dark/a light = "quicknix"' "$CONFIG_FILE"
            fi
        else
            # Add [flavor] section at the end
            echo "" >>"$CONFIG_FILE"
            echo "[flavor]" >>"$CONFIG_FILE"
            echo 'dark  = "quicknix"' >>"$CONFIG_FILE"
            echo 'light = "quicknix"' >>"$CONFIG_FILE"
        fi
    fi
    ;;
...
niri)
    ...
    INCLUDE_LINE='include "./quicknix.kdl"'
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Check if quicknix include already exists (flexible: quotes, ./ prefix)
        if grep -qE 'include\s+["'"'"'](\./)?quicknix\.kdl["'"'"']' "$CONFIG_FILE"; then
            : # Already included
        else
...
hyprland)
    echo "🎨 Applying 'quicknix' theme to Hyprland..."
    CONFIG_DIR="$HOME/.config/hypr"
    CONFIG_FILE="$CONFIG_DIR/hyprland.conf"
    THEME_FILE="$CONFIG_DIR/quicknix/quicknix-colors.conf"
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        mkdir -p "$(dirname "$CONFIG_FILE")"
        echo -e "\n$INCLUDE_LINE\n" >"$CONFIG_FILE"
        echo "Created new config file with quicknix theme."
    else
        # Check if quicknix theme source already exists (flexible matching)
        if grep -qE 'source\s*=\s*.*quicknix.*\.conf' "$CONFIG_FILE"; then
            ...
            # Add the include line to the end of the file
            echo -e "\n$INCLUDE_LINE\n" >>"$CONFIG_FILE"
            echo "✅ Added quicknix theme include to config."
        fi
    fi
...
sway)
    echo "🎨 Applying 'quicknix' theme to Sway..."
    CONFIG_DIR="$HOME/.config/sway"
    CONFIG_FILE="$CONFIG_DIR/config"
    INCLUDE_LINE='include ~/.config/sway/quicknix'
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Check if quicknix include already exists (flexible matching)
        if grep -qE 'include\s+.*quicknix' "$CONFIG_FILE"; then
            echo "Theme already included, skipping modification."
        else
            # Only convert symlink when we actually need to write (NixOS read-only symlinks)
            if [ -L "$CONFIG_FILE" ] && [ ! -w "$CONFIG_FILE" ]; then
...
scroll)
    echo "Applying 'quicknix' theme to Scroll..."
    CONFIG_DIR="$HOME/.config/scroll"
    CONFIG_FILE="$CONFIG_DIR/config"
    INCLUDE_LINE='include ~/.config/scroll/quicknix'
...
mango)
    ...
    THEME_FILE="$CONFIG_DIR/quicknix.conf"
    BACKUP_FILE="$CONFIG_DIR/theme.conf.bak"
    # This sources the quicknix theme file
    ...
    if [ -f "$MAIN_CONFIG" ] && grep -qF "$SOURCE_LINE" "$MAIN_CONFIG"; then
        ...
        if [ -f "$MAIN_CONFIG" ]; then
            echo "" >>"$MAIN_CONFIG"
            echo "# This sources the quicknix theme" >>"$MAIN_CONFIG"
            echo -e "\n$SOURCE_LINE\n" >>"$MAIN_CONFIG"
        else
            echo "# This sources the quicknix theme" >"$MAIN_CONFIG"
            echo -e "\n$SOURCE_LINE\n" >>"$MAIN_CONFIG"
...
btop)
    ...
    if [ -f "$CONFIG_FILE" ]; then
        # Check if theme is already set to quicknix (flexible spacing)
        if grep -qE '^color_theme\s*=\s*"quicknix"' "$CONFIG_FILE"; then
            : # Already correct
        elif grep -qE '^color_theme\s*=' "$CONFIG_FILE"; then
            # Replace existing color_theme line in-place
            sed -i -E 's/^color_theme\s*=.*/color_theme = "quicknix"/' "$CONFIG_FILE"
        else
            echo 'color_theme = "quicknix"' >>"$CONFIG_FILE"
        fi
...
starship)
    ...
    MARKER_BEGIN="# >>> QUICKNIX STARSHIP PALETTE >>>"
    MARKER_END="# <<< QUICKNIX STARSHIP PALETTE <<<"

    # Create config file if it doesn't exist
    if [ ! -f "$CONFIG_FILE" ]; then
        mkdir -p "$(dirname "$CONFIG_FILE")"
        echo 'palette = "quicknix"' > "$CONFIG_FILE"
        echo "" >> "$CONFIG_FILE"
    ...
    # 1. Set palette = "quicknix" at top level
    if grep -qE '^palette\s*=' "$CONFIG_FILE"; then
        sed -i -E 's/^palette\s*=.*/palette = "quicknix"/' "$CONFIG_FILE"
    else
        # Insert after schema line if present, otherwise at line 1
        if grep -qE '^\"\$schema\"' "$CONFIG_FILE"; then
            sed -i '/^\"\$schema\"/a palette = "quicknix"' "$CONFIG_FILE"
        else
            sed -i '1i palette = "quicknix"' "$CONFIG_FILE"
        fi
    fi

    # 2. Remove existing quicknix palette block (between markers)
